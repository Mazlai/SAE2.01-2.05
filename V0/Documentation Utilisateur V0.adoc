...
Reprenons une nouvelle fois le client orange1 crée plus haut, plus précisément son compte. Comme vous pouvez le voir si l'image ci-dessous, le client hercule1 possède un compte, pour le consulter il vous suffit de cliquer dessus puis suivre la flèche rouge.
...
Pour ne pas chnager, reprenons le client hercule1, nous allons effectuer 2 debits (par carte bleue et espèce). Pour choisir le type de retrait il vous suffit de cliquer sur la barre déroulante nommée "Retrait Espèces" puis choisir entre les deux options comme sur l'image ci-dessous :
...
Pour rendre inactif un client, il faut que tous les comptes du client soient clôturés donc nous vous proposons de tous les clôturer en un clic comme sur l'imae ci-dessous
...
==== Si vous êtes arriver à le rendre inactif

Voici l'affichage lorsqu'un client est inactif 

image::/V0/images/ri4euve.png[Etape 4preuve rendre inactif un client]
