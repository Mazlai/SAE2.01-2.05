...
Mais ce ne serait sans oublier le montant dont vous êtes libre de choisir pour rendre le virement fonctionnel. Toutefois, veuillez à vérifier votre découvert autorisé en fonction du solde de votre compte si vous aviez pour ambition de réaliser un virement auprès d'un autre client !

Si vous pensez que vous êtes prêt à réaliser votre virement, alors appuyez sur "Effectuer virement" pour confirmer le virement que vous étiez en train de réaliser jusque-là !

image::/V0/images/interfacevirement.PNG[Etape 2 pour faire un virement de compte à compte]

Comme toute autre opération (créditer/débiter), vous verrez également votre virement s'afficher dans la liste des opérations effectuées depuis ce même compte et il en va de même pour le bénéficiaire du virement !
