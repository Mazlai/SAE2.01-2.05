...
Vient ensuite la convivialité et la simplicité d’utilisation de l’application. L’application doit être constituée d’une interface simple, épurée et ergonomique afin que l’utilisateur puisse l’utiliser de manière sereine, sans difficultés de navigation. 
...
En revanche si la banque autorise un montant de découvert autotirisé, le client pourra retirer dans les négatifs tant qu'il ne dépasse pas le montant du découvert autorisé.
...
• Enfin si le temps nous le permet, il pourra donner une somme d'argent équivalente à la quantité de bitcoin que le client voudra échanger.
